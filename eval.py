...
from datasets import Kitti, get_val_dataloader
        ...
        for cls in CLASSES:
            ...
            for difficulty in [0, 1, 2]:
                ...
                for id in ids:
                    ...
                    cur_det_names = det_result['names']
                    ...
                    total_scores.append(det_result['scores'])
...
def main(args):
    ...
    saved_folder = Path(args.saved_path)
    saved_folder.mkdir(exist_ok=True)
    saved_submit = Path(args.saved_path) / 'submit'
    saved_submit.mkdir(exist_ok=True)
    ...
    with torch.no_grad():
        ...
        for i, data_dict in enumerate(tqdm(val_dataloader)):
            ...
            for j, result in enumerate(batch_results):
                ...
                calib_info = data_dict['batched_calibs'][j]
                tr_velo_to_cam = calib_info['tr_velo_to_cam'].astype(np.float32)
                r0_rect = calib_info['r0_rect'].astype(np.float32)
                P2 = calib_info['P2'].astype(np.float32)
                image_shape = data_dict['batched_image_info'][j]['image_shape']
                ...
                for lidar_bbox, label, score, bbox2d, camera_bbox in \
                    ...
                    format_result['names'].append(LABEL2CLASSES[label])
                    ...
                    format_result['locations'].append(camera_bbox[:3])
                    format_result['rotation_y'].append(camera_bbox[6])
                    format_result['scores'].append(score)
                
                write_label(Path(saved_submit) / f'{idx:06d}.txt', format_result)

                format_results[idx] = {k:np.array(v) for k, v in format_result.items()}
        
        write_pickle(Path(saved_folder) / 'result.pkl', format_results)
    
    print('Evaluating.. Please wait several seconds.')
    do_eval(format_results, val_dataset.data_infos, CLASSES, saved_folder)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--nclasses', type=int, default=config['num_classes'])
